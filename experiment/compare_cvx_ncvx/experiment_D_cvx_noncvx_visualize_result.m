...
resultpath = './results2plot/';

path_to_ALL_RESULT = {[resultpath,'T150_DGN_CVX_ALL_RESULT_K',int2str(K)], ...
    [resultpath,'T150_DGN_ALL_RESULT_K',int2str(K)]};
path_to_index = {[resultpath,'T150_DGN_CVX_result_K',int2str(K)], ...
    [resultpath,'T150_DGN_result_K',int2str(K)]};
...
for tt=1:length(type_acc)
    ...
    sgtitle([text_title,', formulation D convex(left), non-convex(right)'])
    subplot_cnt = 0;
    for type=1:2
