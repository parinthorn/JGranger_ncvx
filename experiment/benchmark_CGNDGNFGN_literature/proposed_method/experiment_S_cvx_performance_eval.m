...
for ii=1:dd
    for jj=1:realization
        fprintf('(%d,%d)\n',ii,jj)
        GTmodel = E{3,3,ii,jj};
        fname = [resultpath,'estim_FGN_cvx_',mname{ii},'percent_lag1_K5_',int2str(jj)];
        load(fname)
...
save([performance_path,'FGN_cvx_result_K5'],'R')
save([performance_path,'FGN_cvx_ALL_RESULT_K5'],'ALL_RESULT')
