...
for jj=1:realization
    fprintf('(%d,%d)\n',ii,jj)
    GTmodel = E{2,jj};
    fname = [resultpath,'LLHcorrected_resultT150_adaptive_formulationC_',mname{ii},'percent_lag3_K',int2str(K),'_',int2str(jj)];
    load(fname)
...
save([performance_path,'LLHcorrected_adaptive_formulation_CT150_result_K',int2str(K)],'R')
save([performance_path,'LLHcorrected_adaptive_formulation_CT150_ALL_RESULT_K',int2str(K)],'ALL_RESULT')
%%
clear R
clear ALL_RESULT
realization = 100;
for jj=1:realization
    fprintf('(%d,%d)\n',ii,jj)
    GTmodel = E{2,jj};
    fname = [resultpath,'LLHcorrected_resultT150_cvx_adaptive_formulationC_',mname{ii},'percent_lag3_K',int2str(K),'_',int2str(jj)];
    load(fname)
%     M = augment_score_old(M,T);
    model_acc = performance_eval(M,GTmodel);
    toggle_list = {'total','common','differential'};
...
save([performance_path,'LLHcorrected_adaptive_formulation_CT150_cvx_result_K',int2str(K)],'R')
save([performance_path,'LLHcorrected_adaptive_formulation_CT150_cvx_ALL_RESULT_K',int2str(K)],'ALL_RESULT')