...
for jj=1:realz
    ...
    model = E{2,jj}; % type D
    y = sim_VAR(model.A,T,1,model.seed,0);
    M = test_cvxformulation_C(y,p_est,GridSize);
    save([outpath,'resultT150_cvx_adaptive_formulationC_',mname{ii},'percent','_lag',int2str(p_est),'_K',int2str(K),'_',int2str(jj)],'M')
