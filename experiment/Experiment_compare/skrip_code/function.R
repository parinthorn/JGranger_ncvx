...
Calc.AUC.from.FP.FN <- function(FP,FN,lambda.path,plot=F,title=NULL){
  ...
  FN.adj <- c(1,FN,0)
  TP.adj <- 1-FN.adj
  Area <- sum((TP.adj[-length(TP.adj)] + TP.adj[-1])*diff(FP.adj)/2)
  if (plot == T){
    plot(FP.adj,TP.adj,main=title)
    lines(c(0,1),c(0,1))
  }
  return(Area)
}
...
Simul.Data.Separate.Main <- function(# USES A LOT OF GLOBAL VARIABLES
  ...
  for (j in 1:sigma.iter){
    for(k in 1:K){
      ...
      for (r in 1:dim(Entity.Sep$Est)[2]){
        ...
        Sep.Est  <- ConvertToMatrix.Full(c(Entity.Sep$Est[,r]),p)
        f.mine <- forecast.mine(as.matrix(GenData.train[(k-1)*p + 1:p,train]), Test = as.matrix(GenData.test[(k-1)*p + 1:p]),Est = Sep.Est, H=h,K=2)
        Pred.Err.vec <- c(Pred.Err.vec,f.mine$MSFE[1])
      }
    }
  }
  ...
  for (k in 1:K){
    ...
    FN.vec <- NULL
    Frob.vec <- NULL
    for (r in 1:length(Entity.list[[k]]$lambda.path)){
      Meas.vec <- Measures.Vec(as.matrix(Entity.list[[k]]$Est[,r]),as.matrix(A.true.vec[1:p^2 +(k-1)*p^2]))
  ...
  return(list(lambda1.picked=lambda1.picked,
              ...
              Frob.Sep=Frob.Sep,
              Sep.Est=Sep.Est,
              AUROC.Sep.Mean=AUROC.Sep.Mean,
              Frob.Sep.Mean=Frob.Sep.Mean,
              Pred.Err.Sep.Mean=Pred.Err.Sep.Mean,
...
Simul.Data.Joint.Main <- function( # USES A LOT OF GLOBAL VARIABLES
  ...
  return(list(Pred.Err.Joint=Pred.Err.Joint,
              ...
              Joint.Est=Joint.Est,
              AUROC.Joint.Mean=AUROC.Joint.Mean,
              Frob.Joint.Mean=Frob.Joint.Mean,
              Pred.Err.Joint.Mean = Pred.Err.Joint.Mean,
