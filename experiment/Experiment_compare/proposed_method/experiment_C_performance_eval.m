...
for ii=2:length(mname)
    for jj=1:realization
        ...
        fname = [resultpath,'LLHcorrected_result_adaptive_formulationC_',mname{ii},'percent_',int2str(jj)];
        load(fname)
...
save([performance_path,'LLHcorrected_adaptive_formulation_C_result'],'R')
save([performance_path,'LLHcorrected_adaptive_formulation_C_ALL_RESULT'],'ALL_RESULT')
%%
clear
clc
inpath = './data_compare/';
