...
for jj=1:3
    for ii=1:3
        extra_link{ii,jj} = setdiff(result.ADHD_index.(namelist{ii}){jj},result.TDC_index.(namelist{ii}){jj});
        missing_link{ii,jj} = setdiff(result.TDC_index.(namelist{ii}){jj},result.ADHD_index.(namelist{ii}){jj});
        missing_grid{ii,jj} = zeros(116);
        extra_grid{ii,jj} = zeros(116);
        missing_grid{ii,jj}(missing_link{ii,jj})=1;
        extra_grid{ii,jj}(extra_link{ii,jj})=1;
        AAL_grid{ii,jj} = zeros(116);
        AAL_grid{ii,jj}(missing_link{ii,jj}) = -1;
        AAL_grid{ii,jj}(extra_link{ii,jj}) = 1;
    end
end
tt = tiledlayout(3,3);
tt.Padding = 'compact';
...
for jj=1:3
    for ii=1:3
        [cause.extra.links,cause.extra.ind] = sort(sum(extra_grid{ii,jj},1),'descend');
        [effect.extra.links,effect.extra.ind] = sort(sum(extra_grid{ii,jj},2),'descend');
        
        [cause.missing.links,cause.missing.ind] = sort(sum(missing_grid{ii,jj},1),'descend');
        [effect.missing.links,effect.missing.ind] = sort(sum(missing_grid{ii,jj},2),'descend');
