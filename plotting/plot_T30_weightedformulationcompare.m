...
diff_den = {'1%','5%'};
plotting_sequence_NCVX = [1,3,5];
plotting_sequence_CVX = [2,4,6];
cnt = 0;
for ii=1:dd
    for tt=1:length(type_acc)
        ...
        sgtitle(['Non-convex(left), convex(right)',', diff density:',diff_den{ii}])
        for ss=1:length(acc_list)
            ...
            subplot(length(acc_list),2,plotting_sequence_NCVX(ss))
            imagesc(val)
...
name_list = {'bic','aicc','eBIC','GIC_2','GIC_3','GIC_4','GIC_5','GIC_6'};%{'bic','aicc'};
resultpath = 'G:/My Drive/0FROM_SHARED_DRIVE/THESIS/formulation_D_result/';
load([resultpath,'formulation_DT30_adaptive_cvx_ALL_RESULT_K',int2str(K)])
load([resultpath,'formulation_DT30_adaptive_cvx_result_K',int2str(K)])
dd = size(ALL_RESULT,1);
realz = size(ALL_RESULT,2);
diff_den = {'1%','5%'};
cnt = 0;
for ii=1:dd
    for tt=1:length(type_acc)
        ...
        for ss=1:length(acc_list)
            ...
            subplot(length(acc_list),2,plotting_sequence_CVX(ss))
            imagesc(val)
