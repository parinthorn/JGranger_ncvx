...
obj = @(u) 0.5*(norm(G*u-b))^2+normpq_adaptive(u(PInd),pp,qq,p,a1)+normpq_adaptive(u(PInd),pp,qq,p*K,a2);
objx0 = obj(x0);
% APG INITIALIZATION
k=1;
xkm1 = x0;
...
while k<IT_MAX
    ...
    if (IS_LINESEARCH) && (k>2)
        ...
        zkp1 = prox_Px_weighted_numerical(yk-ay*(GtGyk_Gtb),a1*ay,a2*ay,PARAMETER);
    ...
    if (obj(zkp1)<=ck-d*norm(zkp1-yk)^2 )
        ...
        if (IS_LINESEARCH) && (k>2)
            ...
            vkp1 = prox_Px_weighted_numerical(xk-ax*(GtGxk_Gtb),a1*ax,a2*ax,PARAMETER);
