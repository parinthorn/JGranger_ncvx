function [x,obj_prox] = select_prox_weighted(z,w1,w2,qq)
prox_obj_fun = @(x) w2*norm(x(:),'fro')^(qq)+sum(w1'.*sqrt(sum(x.^2,1)).^(qq))+ ...
    0.5*norm(x(:)-z(:),'fro')^2;
[p,K] = size(z);
...
%         [x(:,:,jj),prox_obj,flag(jj)] = fix_point_prox(z,nz_ind,a1,a2,qq);
        [x(:,:,jj),prox_obj,flag(jj)] = fix_point_prox_weighted(z,nz_ind,w1,w2,qq);
