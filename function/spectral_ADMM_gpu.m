function [x,L1x,L2x, history] = spectral_ADMM_gpu(G, b,a1,a2,pp,qq,PARAMETER,varargin)
% Spectral ADMM for full Granger Causality with non-convex penalties (formulation S)
...
% The output are
%           x         : solution of the problem in full format
...
L2t = ((L2'));
L1t = ((L1'));
L2tL2 = gpuArray(single(full(L2'*L2))); 
GtG = gpuArray(single(G'*G));
L1tL1 = gpuArray(single(full(L1'*L1)));
...
y1 = gpuArray(single(zeros(np,1))); y2 = gpuArray(single(zeros(nd,1)));
y1hat = y1;
y2hat = y2;
L1x = L1*x;
L2x = L2*x;
...
%     c = Gtb+ P'*(rhotilde*(x2+x3)-(z1+z2));
    c = Gtb+ L1t*double(rho*z1+y1) + L2t*double(rho*z2+y2); 
    
    x = U \ (L \ c);
    % x2-update
    
    L1x = L1*x;
    z1 = prox_pq_eff(L1x-y1/rho,a1/rho,[pp,qq,m1]);
...
% return full result
% x2((x3==0)) = 0;
X = reshape(x,p,K,n^2); % x1 is not full
X2 = reshape(z1,p,K,n^2-n); % x2 is offdiagonal entries only and full
