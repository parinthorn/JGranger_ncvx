function [lambda_max] = lambdamax_grouplasso_adaptive(G,b,block_size,PARAMETER,varargin)
% [lambda_max] = lambdamax_grouplass(G,b,[n p K],P)
%
% lambda_max calculates the maximum value of penalty parameter in the
% problem
%
%   minimize 1/2*|| Gx - b ||_2^2 + \lambda sum(norm(P*x)) [block=block_size]
%
%
...
if mod(n^2*p*K,block_size)==0
    C = reshape(c,block_size,n^2*p*K/block_size);
else
