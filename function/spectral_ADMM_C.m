function [x,L1x,L2x, history] = spectral_ADMM_C(G, b,a,PARAMETER,varargin)
...
y1old = y1;
y2old = y2;
objx0 = 0.5*norm(G*x-b)^2 + a*normpq(z1,2,1,p*K)+a*sum(z2.^2,'all');
...
%     c = Gtb+ P'*(rhotilde*(x2+x3)-(z1+z2));
    ...
    L1x = L1*x;
    z1 = prox_pq_eff(L1x-y1/rho,a/rho,[2,1,p*K]);
    
    % x3-update
    L2x = L2*x;
%     z2 = prox_pq_eff(L2x-y2/rho,a2/rho,[pp,qq,m2]);
    z2 = (rho*L2x-y2)/(2*a+rho);
    ...
    obj = 0.5*norm(G*x-b)^2 + a*normpq(z1,2,1,p*K)+a*sum(z2.^2,'all');
