function [x,L1x,L2x, history] = spectral_ADMM(G, b,a1,a2,pp,qq,PARAMETER,varargin)
...
%             disp('----------------PRIMAL INFEASIBLE----------------')
    ...
    if (IS_ADAPTIVE==0)&&(PARAMETER.IS_ADAPTIVE==1)&&(sum(history.nz_count(counting_nz_index))==history.nz_count(k)*length(counting_nz_index))
        history.fit = 0.5*norm(G*x-b)^2;
        break
    end
    xold = x;
