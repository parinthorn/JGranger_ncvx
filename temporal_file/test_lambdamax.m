function M = test_lambdamax(y,P,varargin)
...
Lmax = lambdamax_grouplasso_v2(gc,yc,p,[n ,p ,K]);
Lambda = Lambda*Lmax;
Lambda(1) = [];
Lambda = [Lambda Lambda(end)*1.01];
xLS = gc\yc;
if T>n*p
  init_cvx = 0;
else
...
for ii=1:GridSize
    a = Lambda(ii);
        ...
        [x_reg,~, ~, history] = spectral_ADMM(gc, yc,a, 0,2,1, ALG_PARAMETER,x0);
