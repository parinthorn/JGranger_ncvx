function [M_out] = augment_score(M,T,toggle,varargin)
if isempty(varargin)
    is_hold =1;
else
    is_hold = 0;
    fname = varargin{1}; %similar, lasso, group_lasso
end
name_list = {'bic','aic','aicc','eBIC','GIC_2','GIC_3','GIC_4','GIC_5','GIC_6'};
M_out = M;
M_out.LLH_type = toggle;
if prod(size(M.model))==M.GridSize^2
    ...
    for ii=1:M.GridSize
        for jj=1:M.GridSize
            if is_hold
                ...
                switch fname
                    case 'similar'
                        df = eval_S_df(M.model(ii,jj).A);
                    case 'lasso'
                        df = length(find((M.model(ii,jj).A)~=0));
                    case 'group_lasso'
            ...
            switch toggle
                case 'LLH_full'
                    LLH = M.model(ii,jj).stat.model_selection_score.LLH_full;
                case 'LLH_hetero'
                    LLH = M.model(ii,jj).stat.model_selection_score.LLH_hetero;
                case 'LLH_homo'
                    LLH = M.model(ii,jj).stat.model_selection_score.LLH_homo;
            end
    ...
    for nn=1:length(name_list)
        tmp = [M_out.model.stat]; tmp = [tmp.model_selection_score];tmp=(reshape(([tmp.(name_list{nn})]),[M.GridSize,M.GridSize]));
        [~,M_out.index.(name_list{nn})] = min(tmp(:));
    end
else
    tmp = [M.model.stat]; tmp = [tmp.model_selection_score]; tmp=(reshape(([tmp.df]),[1,M.GridSize]));
    n = size(M.model(1).A,1);
    p = size(M.model(1).A,3);
    K = size(M.model(1).A,4);
    for ii=1:M.GridSize
        ...
        switch toggle
            ...
            case 'LLH_hetero'
                LLH = M.model(ii).stat.model_selection_score.LLH_hetero;
            case 'LLH_homo'
                LLH = M.model(ii).stat.model_selection_score.LLH_homo;
        end
        ...
        score.aic = fitting + 2*df;
        score.aicc = score.aic + (2*df^2+2*df)/(Num-df-1);
        M_out.model(ii).stat.model_selection_score.df = df;
        for nn=1:length(name_list)
            M_out.model(ii).stat.model_selection_score.(name_list{nn}) =  score.(name_list{nn});
        end
    end
    for nn=1:length(name_list)
        tmp = [M_out.model.stat]; tmp = [tmp.model_selection_score];tmp=(reshape(([tmp.(name_list{nn})]),[1,M.GridSize]));
        [~,M_out.index.(name_list{nn})] = min(tmp(:));
    end
end
end