...
%         M = coherent_GC(y,P,p_est,GridSize);
        load([outpath,'magda_formulationC_',mname{cnt},'percent','_',int2str(jj)],'M')
        toggle = 'LLH_full';
        data_concat = 0;
        M = correction_Magda(y,M,p_est,GridSize,toggle,data_concat);        
        save([outpath,'LLHcorrected_magda_formulationC_',mname{cnt},'percent','_',int2str(jj)],'M')
