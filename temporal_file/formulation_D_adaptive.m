function M = formulation_D_adaptive(y,P,varargin)
...
xLS = gc\yc;
weight_1 = 1./(sqrt(sum(reshape(P*xLS,[p,(n^2-n)*K]).^2,1))');
weight_2 = 1./(sqrt(sum(reshape(P*xLS,[p*K,n^2-n]).^2,1))');
...
Lambda_2 = Lambda*Lmax_2.*weight_2;
% xLS = x_cheat;
if T>n*p
  init_cvx = 0;
else
